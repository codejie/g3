...
- **types/model.ts**: 模型相关类型定义
  ...
  - `Provider`: 提供者实体（id, provider_id, npm?, options[]）
...
- **modules/model/**: Model模块
 - model.ts: 模型数据模型（Provider/Model CRUD + Options管理）
 - handler.ts: 业务处理（获取列表/增删改Provider/增删改Model），所有变更操作后调用syncConfigProvider()
 - routes.ts: Fastify路由定义（7路由+JSON Schema+authenticateAdmin）
 - configSync.ts: 配置文件同步（syncConfigProvider: 从DB读取providers/models → 构建config.json的provider字段 → 写入opencode配置文件）
