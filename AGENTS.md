...
- ChatInput.vue: 消息输入组件，包含两层快捷提示词（Keyword→Prompt，DB驱动）、发送按钮、Agent模式下拉选择（BUILD/PLAN），通过`agentModeChange`事件传递选择的agent值
...
- keywordStore.ts: 快捷提示词状态管理（关键词列表、活跃关键词、Prompt选择）
...
- **types/keyword.ts**: 快捷提示词相关类型定义
- `Keyword`: 关键词实体（id, keyword, description?, disabled, order, type, created_at, updated_at）
- `Prompt`: 提示词内容实体（id, keyword_id, label, prompt, description?, disabled, order, created_at, updated_at）
- `KeywordRow/PromptRow`: DB行类型
- `GetKeywordsRequest/GetKeywordsResponse`: 获取关键词列表
- `AddKeywordRequest/AddKeywordResponse`: 添加关键词
- `DeleteKeywordRequest/DeleteKeywordResponse`: 删除关键词
- `UpdateKeywordRequest/UpdateKeywordResponse`: 更新关键词
- `AddPromptRequest/AddPromptResponse`: 添加提示词内容
- `DeletePromptRequest/DeletePromptResponse`: 删除提示词内容
- `UpdatePromptRequest/UpdatePromptResponse`: 更新提示词内容
- **types/session.ts**: 会话管理相关类型定义
- `SessionItem`: 会话项实体（id, title, directory, created, updated, project_name, project_type, project_status, user_name）
- `GetSessionsRequest/GetSessionsResponse`: 获取会话列表
- `GetAutoCleanRequest/GetAutoCleanResponse`: 获取自动清理状态
- `SetAutoCleanRequest/SetAutoCleanResponse`: 设置自动清理状态
...
- **api/keywordApi.ts**: 快捷提示词API（getKeywords, addKeyword, deleteKeyword, updateKeyword, addPrompt, deletePrompt, updatePrompt）
- **api/sessionApi.ts**: 会话管理API（list, getAutoClean, setAutoClean）
...
- 快捷提示词表(keywords)、提示词内容表(prompts)
- 系统配置表(system_config)，键值对结构存储系统级配置（如autoCleanEnabled）
- 索引优化、初始数据（admin/tester用户，8关键词×3提示词种子数据，autoCleanEnabled='0'默认关闭）
...
- **modules/keyword/**: Keyword模块（快捷提示词）
- model.ts: 关键词数据模型（Keyword/Prompt CRUD，含级联删除）
- handler.ts: 业务处理（获取列表/增删改Keyword/增删改Prompt，含字段验证）
- routes.ts: Fastify路由定义（7路由+JSON Schema+权限控制：list=User，变更=Admin）
- **modules/session/**: Session模块（会话管理）
- handler.ts: 业务处理（会话列表查询、自动清理状态getAutoCleanHandler/setAutoCleanHandler、getAutoCleanEnabled/setAutoCleanEnabled读写system_config、cleanupUnboundSessions清理未关联项目的会话）
- routes.ts: Fastify路由定义（3路由+JSON Schema+authenticateAdmin：/api/session/list, /api/session/auto-clean/get, /api/session/auto-clean/set）
- **index.ts**额外功能：AutoClean定时器（1分钟检查间隔，30分钟清理间隔，启用时清理未关联项目的OpenCode sessions，timer.unref()不阻止进程退出，gracefulShutdown时clearInterval）
...
- prompts表：存储快捷提示词内容
- id: TEXT PRIMARY KEY（UUID）
- keyword_id: TEXT NOT NULL（外键关联keywords，ON DELETE CASCADE）
- label: TEXT NOT NULL
...
- created_at: INTEGER DEFAULT (strftime('%s', 'now'))
- updated_at: INTEGER DEFAULT (strftime('%s', 'now'))
- system_config表：存储系统级配置（键值对结构）
- key: TEXT PRIMARY KEY（配置键名，如autoCleanEnabled）
- value: TEXT NOT NULL（配置值，如'0'或'1'）
- updated_at: INTEGER DEFAULT (strftime('%s', 'now'))
...
│ │ ├── skillApi.ts # 技能API
│ │ ├── systemApi.ts # 系统API
│ │ ├── keywordApi.ts # 快捷提示词API
│ │ └── sessionApi.ts # 会话管理API
│ └── types/ # 类型定义
│ ├── index.ts # 导出文件
│ ├── common.ts # 公共类型定义
│ ├── user.ts # 用户相关类型
...
│ │ │ ├── handler.ts # 业务处理
│ │ │ └── routes.ts # 路由定义
│ │ │ └── keyword/ # Keyword模块（快捷提示词）
│ │ │ ├── model.ts # 关键词数据模型
│ │ │ ├── handler.ts # 业务处理
│ │ │ └── routes.ts # 路由定义
│ │ │ └── session/ # Session模块（会话管理）
│ │ │ ├── handler.ts # 业务处理（含自动清理逻辑）
│ │ │ └── routes.ts # 路由定义
│ │ └── utils/ # 工具函数
│   │       ├── db.ts # 数据库连接转发
│   │       └── logger.ts # 日志工具
...
# 使用手册
- [Backend 使用手册](docs/backend_manual.md): 后端服务的安装、配置、API接口、认证机制、部署等完整说明
- [Frontend 使用手册](docs/frontend_manual.md): 前端应用的安装、配置、页面功能、状态管理、API层、部署等完整说明
